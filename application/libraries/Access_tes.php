...
class Access_tes{
	...
	function login($username, $password){
		...
		if($result){
			if($password === $result->user_password and $result->user_detail==1){
				...
				$data['user_detail'] = 1;
				$this->users_model->update('user_name', $username, $data);
				return 1;
			}else if($result->user_detail==0){
				return 4;
			}
			else if($result->user_detail==7){
				return 7;
			}else if($result->user_detail==99){
				return 99;
			}
			else{
				return 2;
