...
class Welcome extends CI_Controller {
            ...
            if($this->agent->browser()=='Internet Explorer' ){
				...
				if($this->agent->is_mobile()){
					...
					if($query->row()->konfigurasi_isi=="ya"){
						...
						if(strpos($agent, ' xyzcbtonline')==false){
							$akses_cbt = 0;
						}
					}
				}
	...
	function check_login(){	
		...
		if($login==1){
			...
			$this->form_validation->set_message('check_login','Belum saatnya anda mengikuti ujian');
			return FALSE;
		}else if($login==7){
			$this->form_validation->set_message('check_login','Anda terdeteksi menutup aplikasi CBT. Silahkan hubungi operator!');
			return FALSE;
		}else if($login==99){
			$this->form_validation->set_message('check_login','User anda sedang aktif di device lain. Silahkan logout!');
			return FALSE;
		}else{
